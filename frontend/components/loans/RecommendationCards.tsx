...
import { useDebtRecommendations } from '@/hooks/useDebtIntelligence';
import { RecommendationSeverity } from '@/types';
import { cn } from '@/lib/utils';

const SEVERITY_CONFIG: Record<RecommendationSeverity, { icon: typeof AlertTriangle; text: string; bg: string; border: string }> = {
...
export function RecommendationCards() {
  const { data, isLoading } = useDebtRecommendations();
  ...
  return (
    <div className="glass-card rounded-2xl p-5 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-text-primary">Recommended Action Plan</h3>
      </div>
