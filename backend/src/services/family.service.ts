...
export async function getLoanStrategy(userId: string) {
  const loans = await prisma.loan.findMany({
    where: { userId, isActive: true },
    include: { member: { select: { id: true, name: true, color: true } } },
  });

  const withStats = loans.map((l) => {
    const remaining = Number(l.remainingBalance);
    const emi       = Number(l.emi);
    const r         = Number(l.interestRate) / 100 / 12;
    const monthlyInterest  = r > 0 ? remaining * r : 0;
    const monthlyPrincipal = Math.max(0, emi - monthlyInterest);
    const monthsToPayoff   = r > 0 && emi > monthlyInterest
      ? Math.ceil(-Math.log(1 - (r * remaining) / emi) / Math.log(1 + r))
      : emi > 0 ? Math.ceil(remaining / emi) : 999;

    return { id: l.id, name: l.name, remainingBalance: remaining, emi, interestRate: Number(l.interestRate), monthlyInterest, monthlyPrincipal, monthsToPayoff, member: l.member };
  });

  const avalanche = [...withStats].sort((a, b) => b.interestRate - a.interestRate);
  ...
  return {
    loans: withStats,
    avalancheOrder:         avalanche.map((l) => l.id),
    snowballOrder:          snowball.map((l) => l.id),
    totalMonthlyInterest,
    baseMonths:             base.months,
    avalancheMonthsWith5k:  avl5k.months,
    snowballMonthsWith5k:   snow5k.months,
    interestSavedAvalanche: Math.max(0, base.totalInterest - avl5k.totalInterest),
    interestSavedSnowball:  Math.max(0, base.totalInterest - snow5k.totalInterest),
  };
}
